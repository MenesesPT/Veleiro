...
const uint32_t LoRa::bw[10] = {7800, 10400, 15600, 20800, 31250, 41700, 62500, 125000, 250000, 500000};

LoRa::LoRa(uint8_t spi, uint8_t ss, uint8_t dio0, uint8_t rst) : _spi_channel(spi), _ss_pin(ss), _dio0_pin(dio0), _rst_pin(rst) {}

LoRa *LoRa::setSpreadFactor(LoRa::sf_t sf) {
  sleep();
  sf = sf < SF_6 ? SF_6 : sf > SF_12 ? SF_12 : sf;
  if (sf == SF_6) {
...
LoRa::sf_t LoRa::getSpreadFactor() { return (LoRa::sf_t)(readRegister(REG_MODEM_CONFIG2) >> 4); }
...
uint32_t LoRa::getFrequency() {
  uint64_t frf = (readRegister(REG_FRF_MSB) << 16 | readRegister(REG_FRF_MID) << 8 | readRegister(REG_FRF_LSB) << 0);
...
LoRa::bw_t LoRa::getBandwidth() { return (LoRa::bw_t)(readRegister(REG_MODEM_CONFIG1) >> 4); }
...
uint8_t LoRa::getTXPower() {
  ...
  double pmax = pa_config & PA_BOOST ? 17.0 : 10.8 + 0.6 * (pa_config & 0x38);
...
LoRa::hm_t LoRa::getHeaderMode() { return (LoRa::hm_t)(readRegister(REG_MODEM_CONFIG1) & 0x01); }
...
LoRa::cr_t LoRa::getCodingRate() { return (LoRa::cr_t)((readRegister(REG_MODEM_CONFIG1) & 0x0E) >> 1); }
...
uint8_t LoRa::getSyncWord() { return readRegister(REG_SYNC_WORD); }
...
bool LoRa::getLNABoost() { return (readRegister(REG_LNA) & 0x03) == 0x03; }

int LoRa::getFrequencyError() {
  uint32_t fei = (readRegister(REG_FEI_MSB) & 0x0F) << 16 | readRegister(REG_FEI_MID) << 8 | readRegister(REG_FEI_LSB);
...
uint8_t LoRa::getOpMode() { return readRegister(REG_OPMODE) & OPMODE_MASK; }
...
bool LoRa::packetAvailable() {
  setHeaderMode(HM_EXPLICIT);
  setOpMode(OPMODE_RX);
  uint8_t irq_flags = readRegister(REG_IRQ_FLAGS);
  // writeRegister(REG_IRQ_FLAGS, irq_flags);  // clear IRQs
  return (irq_flags & IRQ_RXDONE_MASK) && ((irq_flags & IRQ_CRCERR_MASK) == 0);
}
