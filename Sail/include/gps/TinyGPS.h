...
public:
  bool isValid() const { return valid; }
  bool isUpdated() const { return updated; }
  unsigned long age() const { return valid ? millis() - lastCommitTime : (unsigned long)ULONG_MAX; }
...
public:
  ...
  static unsigned long parseDecimal(const char *term);
  static void parseDegrees(const char *term, RawDegrees &deg);

  unsigned long charsProcessed() const { return encodedCharCount; }
  unsigned long sentencesWithFix() const { return sentencesWithFixCount; }
  unsigned long failedChecksum() const { return failedChecksumCount; }
  unsigned long passedChecksum() const { return passedChecksumCount; }
...
private:
  ...
  unsigned char curSentenceType;
  unsigned char curTermNumber;
  unsigned char curTermOffset;
  bool sentenceHasFix;
